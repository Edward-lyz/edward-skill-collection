---
recipe: network_traffic_map
display_name: Network Devices Traffic Heatmap
...
```yaml
moduleName: network_traffic_map
displayName: Network Devices Traffic Heatmap
description: |-
  This recipe displays heatmaps of sent traffic, received traffic, and congestion events for network devices.
arguments:
- name: INPUT
...
- name: BYTES_THRESHOLD
  cliOption: --bytes-threshold
  type: int
  title: Bytes Threshold
  default: 0
  description: |-
    Number of bytes threshold for network traffic. Network devices are included in the network traffic plots if at least one sample of bytes traffic for the device is equal or greater to this value. This threshold applies only to bytes received and sent heatmaps.
- name: HIDE_INACTIVE
  cliOption: --hide-inactive
  type: flag
  default: false
  title: Hide Inactive
  description: |-
    Exclude devices with zero traffic from the results.
