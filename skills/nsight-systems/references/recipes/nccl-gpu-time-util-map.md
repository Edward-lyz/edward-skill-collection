---
recipe: nccl_gpu_time_util_map
display_name: NCCL GPU Time Utilization Heatmap
...
This recipe calculates the percentage of time that communication and compute
 kernels were running, as well as their overlap. Communication kernels are
 identified by the 'nccl' prefix. Note that the percentage refers to time
 utilization and does not account for the actual hardware resource utilization.

```yaml
moduleName: nccl_gpu_time_util_map
displayName: NCCL GPU Time Utilization Heatmap
description: |-
  This recipe calculates the percentage of time that communication and compute kernels were running, as well as their overlap. Communication kernels are identified by the 'nccl' prefix. Note that the percentage refers to time utilization and does not account for the actual hardware resource utilization.
arguments:
- name: INPUT
  cliOption: --input
  type: process_input
