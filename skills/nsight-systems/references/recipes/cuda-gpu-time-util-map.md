---
recipe: cuda_gpu_time_util_map
display_name: CUDA GPU Time Utilization Heatmap
...
This recipe calculates the percentage of time that CUDA kernels were running.
 Note that this refers to time utilization and does not account for the actual
 hardware resource utilization.

```yaml
moduleName: cuda_gpu_time_util_map
displayName: CUDA GPU Time Utilization Heatmap
description: |-
  This recipe calculates the percentage of time that CUDA kernels were running. Note that this refers to time utilization and does not account for the actual hardware resource utilization.
arguments:
- name: INPUT
  cliOption: --input
  type: process_input
...
- name: CUMULATIVE
  cliOption: --cumulative
  type: flag
  default: false
  title: Cumulative
  description: |-
    Add up the utilization of all kernels within each bin. The utilization can exceed 100% if multiple kernels are running concurrently
- name: FILTER_TIME
