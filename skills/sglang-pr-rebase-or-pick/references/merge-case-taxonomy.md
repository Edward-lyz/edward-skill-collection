...
确认某个 fork 能力不再需要之后，做法是跟着上游一起删干净，而不是留一份没人调用的实现
等下次合并再冲突一遍。允许删的判据有三条，缺一条就不许动：

1. 最终树里没有任何模块 import 它（`orphan_scan.py` 的 importer 检查）；
2. 它的名字在别处的字符串里也不出现，排除 registry / importlib 动态加载；
3. 它不读任何发版 YAML 真正传进来的开关——这一条是安全网，把「场内还在用」和「代码里
   看起来没人用」区分开，所以 `--deploy-yaml` 要指向真实发版的那几个 YAML，而不是示例。

三条都满足才是 ORPHAN，直接删文件，删完重跑 import 门禁确认没有悬空引用。名字只出现在
字符串里的记 DYNAMIC-MAYBE，人工确认后再动。删除结果写进 CR 描述并点名相关 owner。

实例：本次合并树里 48 个 fork-only 模块全部 IN-USE，一个 ORPHAN 都没有——那 10 条能力
的定义和调用方是被合并一起换掉的，没有留下残骸，所以「跟随删除」这一步无事可做。
